...
def main() -> None:
    ...
    try:
        if Path(real_upload.name).suffix.lower() in EXCEL_SUFFIXES:
            ...
            real_sheet = upload_left.selectbox("Measured Excel sheet", real_sheets, key="real_sheet")
        if Path(simulated_upload.name).suffix.lower() in EXCEL_SUFFIXES:
            ...
            simulated_sheet = upload_right.selectbox("Simulation Excel sheet", simulated_sheets, key="sim_sheet")
